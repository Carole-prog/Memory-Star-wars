...
class Tile{
	...
	display(){
		var carte = document.getElementById('carte-'+(this.x*3+this.y).toString() );
		...
		if(this.id>0){
			if(this.isRevealed == false){
				...
				carte.setAttribute('src',('../../Images/Face/Facile(3x3)/'+this.id.toString()+'.png'));
			}
		}
...
class Board {
	...
	display(){
		...
		for( i=0 ; i < this.largeur ; i+=1 ){
					...
					if(joueur == joueur1){
						...
						if(board.tiles[index].id > 0 && !board.tiles[index].isRevealed && reveal.length < 2){
							...
							if (reveal.length == 2){
								...
								if (board.count() < 2){
									if (score_IA> score_joueur1){
										plateau.innerHTML = "<h1> Gagnant : IA  ! </h1>";
									}else if(score_IA == score_joueur1){
										plateau.innerHTML = "<h1> On arrive pas à battre un robot ?  </h1>"
									}else{
			...
			face.setAttribute('id','carte-'+(i*3+j).toString());
